...
async def Krypton_stats():
    while True:
        ...
        sv_ch = client.get_channel(1189617424543260772)
        users_ch = client.get_channel(1189617443161772172)
...
@tree.command(name="embed", description="Create A Embed Using FosterLK")
...
async def on_command_completion(context: Context) -> None:
    ...
    if context.guild is not None:
        try:
            ...
            embed.add_field(
                name="<a:lh_arrow:1189498009067720775> Command Name :",
                value=f"{executed_command}",
                inline=False)
            embed.add_field(
                name="<a:lh_arrow:1189498009067720775> Command Executed By :",
                value=
                f"{context.author} | ID: [{context.author.id}](https://discord.com/users/{context.author.id})",
                inline=False)
            embed.add_field(
                name="<a:lh_arrow:1189498009067720775> Command Executed In :",
                value=
                f"{context.guild.name}  | ID: [{context.guild.id}](https://discord.com/users/{context.author.id})",
                inline=False)
            embed.add_field(
                name=
                "<a:lh_arrow:1189498009067720775> Command Executed In Channel :",
                value=
                f"{context.channel.name}  | ID: [{context.channel.id}](https://discord.com/channel/{context.channel.id})",
                inline=False)
            embed.set_footer(text="Powered By FosterLK Digital Solutions",
        ...
        try:
            ...
            embed1.add_field(
                ...
                f"{context.author} | ID: [{context.author.id}](https://discord.com/users/{context.author.id})",
                inline=False)
            embed1.set_footer(text="Powered By FosterLK Digital Solutions",
                              icon_url=client.user.display_avatar.url)
