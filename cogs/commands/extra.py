...
cluster = motor.motor_asyncio.AsyncIOMotorClient(
    "mongodb+srv://udiyasl2009:2lkKK6hEQlvEZ6Hs@FosterLK.vniu9zf.mongodb.net/?retryWrites=true&w=majority"
)
...
class Utility(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.db = self.connection["FosterLK"]["servers"]
    ...
    async def stats(self, ctx):
        """Shows some usefull information about FosterLK"""
        ...
        embed = discord.Embed(
            color=0x2f3136,
            description=
            "[Invite](https://discord.com/oauth2/authorize?client_id=1189212094592196691&permissions=2113268958&scope=bot) ● [Support Server](https://discord.gg/HPwNkKsSUJ)"
        )
        ...
        jashu = await self.bot.fetch_user(789735726677229594)
        siva = await self.bot.fetch_user(984726311064989797)
  

        embed.add_field(
            name='﹒DEVELOPERS',
            value=
            f"[{Lyrox}](https://discord.com/users/996060367585300490)"
        )

        embed.set_author(name=f"{self.bot.user.name} Stats",
                         icon_url=self.bot.user.display_avatar.url)
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
        embed.set_footer(text='Thanks For Using FosterLK',
                         icon_url=self.bot.user.display_avatar.url)
    ...
    async def invite(self, ctx: commands.Context):
        embed = discord.Embed(
            description=
            "> • [Click Here To Invite FosterLK To Your Server](https://discord.com/oauth2/authorize?client_id=1189212094592196691&permissions=2113268958&scope=bot)\n> • [Click Here To Join My Support Server](https://discord.gg/HPwNkKsSUJ)",
            color=0x2f3136)
...
**Discord.py Version:** {discord.__version__}
    ...
    async def serverinfo(self, ctx: commands.Context):
        ...
        embed.add_field(
            name="**__About__**",
            value=
            f"**Name : ** {guild.name}\n**ID :** {guild.id}\n**Owner <a:lh_owner:1189498119872839782> :** {guild.owner} (<@{guild.owner_id}>)\n**Created At : ** <t:{c_at}:F>\n**Members :** {len(guild.members)}",
            inline=False)
        ...
        if guild.features:
            embed.add_field(
                name="**__Features__**",
                value="\n".join([
                    f"<a:lh_arrow:1189498009067720775> : {feature.replace('_',' ').title()}"
...
**Total Emoji/Stickers :** {total_emojis}
        ...
        embed.add_field(
            ...
            value=
            f"Level : {guild.premium_tier} [<a:lh_booster:1189498102193856622> {guild.premium_subscription_count} Boosts ]\nBooster Role : <@&{guild.premium_subscriber_role.id}>",
            inline=False)
    ...
    async def _userinfo(self,
        ...
        if member.public_flags.hypesquad:
            badges += "<:HypeSquad_Events39:1189584402527821874> "
        if member.public_flags.hypesquad_balance:
            badges += "<:Icon_Hypesquad_Balance:1189584040156745739> "
        if member.public_flags.hypesquad_bravery:
            badges += "<:Icon_Hypesquad_Bravery:1189584050755735663> "
        if member.public_flags.hypesquad_brilliance:
            badges += "<:Icon_Hypesquad_Brilliance:1189584058456485998> "
        if member.public_flags.early_supporter:
            badges += "<:Discord_Early_Supporter73:1189584035840794635> "
        if member.public_flags.active_developer:
            badges += "<:lh_developer:1189498048691318844> "
        if member.public_flags.verified_bot_developer:
            badges += "<:verifed_bot_developer:1189584086331830393> "
        if member.public_flags.discord_certified_moderator:
            badges += "<:Moderator:1189584065364504767> "
        if member.public_flags.staff:
            badges += "<:Staff:1189584078266187888> "
        if member.public_flags.partner:
            badges += "<:Partnered_Server_Owner73:1189584071437860966> "
...
**Color:** {member.color if member.color else '000000'}
    ...
    async def status(self, ctx, member: discord.Member = None):
        ...
        await ctx.send(embed=discord.Embed(
            title="**<a:lh_magic:1189498059172880444> | status**",
    ...
    async def emoji(self, ctx, emoji: discord.Emoji):
        return await ctx.send(embed=discord.Embed(
            title="**<a:lh_magic:1189498059172880444> | emoji**",
            description="emoji: %s\nid: **`%s`**" % (emoji, emoji.id),
            color=0x2f3136))
    ...
    async def removeemoji(self, ctx, emoji: discord.Emoji):
        await emoji.delete()
        await ctx.send(
            "**<a:lh_checkout:1189498036913700956> emoji has been deleted.**")
    ...
    async def _badges(self, ctx, user: Optional[discord.User] = None):
        ...
        if mem.public_flags.hypesquad:
            ...
            badges += "<:Icon_Hypesquad_Balance:1189584040156745739> *HypeSquad Balance*\n"

        elif mem.public_flags.hypesquad_bravery:
            badges += "<:Icon_Hypesquad_Bravery:1189584050755735663> *HypeSquad Bravery*\n"
        elif mem.public_flags.hypesquad_brilliance:
            badges += "<:Icon_Hypesquad_Brilliance:1189584058456485998> *Hypesquad Brilliance*\n"
        if mem.public_flags.early_supporter:
            badges += "<:Discord_Early_Supporter73:1189584035840794635> *Early Supporter*\n"
        elif mem.public_flags.verified_bot_developer:
            badges += "<:verifed_bot_developer:1189584086331830393> *Verified Bot Developer*\n"
        elif mem.public_flags.active_developer:
            badges += "<:lh_developer:1189498048691318844> *Active Developer*\n"
        if badges == "":
            badges = "None"
        if bdgs == []:
            ...
            embed2.add_field(
                name="**Bot Badges:**",
                value="<:lh_no:1189499009707032576> NO BADGES",
                inline=False)
    ...
    async def ping(self, ctx):
        embed = discord.Embed(
            title="""<a:_Online_Ping_:1189586938026532924> Ping <a:_Online_Ping_:1189586938026532924>""",
