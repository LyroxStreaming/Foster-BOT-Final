...
class Automod(Cog):
    ...
    async def _antiraid(self, ctx: commands.Context):
        ...
        spam = data["antiSpam"]
        link = data["antiLink"]
        embed = discord.Embed(title="FosterLK | Automod Commands",
                              color=0x2f3136)
        embed.add_field(
            name="<a:lh_arrow:1189498009067720775> antispam on/off",
            value=f"Enables/Disables antispam feature\nCurrently Its {spam}",
            inline=False)
        embed.add_field(
            name="<a:lh_arrow:1189498009067720775> antilink on/off",
    ...
    async def _antispam(self, ctx: commands.Context, type: str):
        ...
        if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
            if onOroff == "on":
                if data["antiSpam"] is True:
                    hacker = discord.Embed(
                        description=
                        f"<a:sirenred:1182717716747010129> | Anti-Spam is already enabled for **`{ctx.guild.name}`**",
                        color=0x2f3136)
                    ...
                    hacker1 = discord.Embed(
                        ...
                        description=
                        f"<a:lh_arrow:1189498009067720775> | Successfully enabled anti-spam for **`{ctx.guild.name}`**",
                        color=0x2f3136)
                ...
                hacker2 = discord.Embed(
                    description=
                    f"<a:lh_arrow:1189498009067720775> | Successfully disabled anti-spam for **`{ctx.guild.name}`**",
                    color=0x2f3136)
                await ctx.reply(embed=hacker2, mention_author=False)
            else:
                hacker3 = discord.Embed(
                    description=
                    f"<a:sirenred:1182717716747010129> | Invalid Type.\nIt Should Be On/Off",
                    color=0x2f3136)
            ...
            hacker5 = discord.Embed(
                color=0x2f3136,
                title="FosterLK Security",
                description=
                f"<a:sirenred:1182717716747010129> | Only owner of the server can run this command"
            )
        ...
        if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
            if onOroff == "on":
                if data["antiLink"] is True:
                    hacker = discord.Embed(
                        description=
                        f"<a:sirenred:1182717716747010129> | Anti-link is already enabled for **`{ctx.guild.name}`**",
                        color=0x2f3136)
                    await ctx.reply(embed=hacker, mention_author=False)
                else:
                    data["antiLink"] = True
                    ...
                    hacker1 = discord.Embed(
                        description=
                        f"<a:lh_arrow:1189498009067720775> | Successfully enabled anti-link for **`{ctx.guild.name}`**",
                        color=0x2f3136)
                    await ctx.reply(embed=hacker1, mention_author=False)
                ...
                hacker2 = discord.Embed(
                    description=
                    f"<a:lh_arrow:1189498009067720775> | Successfully disabled anti-link for **`{ctx.guild.name}`**",
                    color=0x2f3136)
                await ctx.reply(embed=hacker2, mention_author=False)
            else:
                hacker3 = discord.Embed(
