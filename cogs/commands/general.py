...
Aliases""")
    ...
    @commands.command(name="users", help="check users of FosterLK .")
    @blacklist_check()
    @ignore_check()
    async def users(self, ctx: commands.Context):
        embed = discord.Embed(
            title=f"**Users:**",
            description=
            f"**{len(set(self.bot.get_all_members()))} Users Of FosterLK**",
            color=0x2f3136)
        await ctx.send(embed=embed)
    ...
    async def pikachu(self, ctx: commands.Context):
        ...
        embed.set_footer(name="Made By ~ mtx.lyrox", icon_url="")
